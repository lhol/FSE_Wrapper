...
namespace Fse.Net
{
    public static class FseInterop
    {
        ...
        /// Reflects the hard limit in FSE_compress() (srcSize &lt;= 2 → returns 0).
        /// </summary>
        public const int MinCompressSize = 3;
        ...
        /// <summary>
        /// Compresses <paramref name="input"/> using the FSE entropy coder.
        /// </summary>
        ...
        /// <exception cref="InvalidOperationException">If the data is not compressible by FSE
        /// (e.g. single-symbol, RLE, or already-compressed data).</exception>
        public static byte[] Compress(byte[] input)
        {
            if (input == null) throw new ArgumentNullException(nameof(input));
            if (input.Length < MinCompressSize)
                throw new ArgumentException(
                    $"input too small for FSE compression: {input.Length} bytes " +
            ...
            if ((ulong)cSize == 0)
                throw new InvalidOperationException(
                    $"FSE compression produced no output: input ({input.Length} bytes) " +
                    "may not be compressible (single-symbol, RLE, or random data)");
        ...
        /// </summary>
        /// <param name="compressed">FSE-compressed bytes.</param>
        ...
        public static byte[] Decompress(byte[] compressed, int expectedSize)
        {
            ...
            if ((ulong)dSize != (ulong)expectedSize)
                throw new InvalidOperationException(
                    $"FSE decompression failed: corrupt input or wrong expectedSize ({expectedSize})");
            return dst;
        }
    }
}
